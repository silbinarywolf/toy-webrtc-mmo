...
type sampleStructFormat struct {
	Bool                 bool
	Int                  int
	Int32                int32
	Int64                int64
	...
	Float32              float32
	Float64              float64
	SliceOfPointerStruct []*sampleStructFormat
	SampleStructEmbeddedStruct
}

type SampleStructEmbeddedStruct struct {
	Int int
}

var testData = []sampleStructFormat{
	{
		// test all zeroed data
	},
	{
		Bool:    true,
		...
		SampleStructEmbeddedStruct: SampleStructEmbeddedStruct{
			Int: 9223372036854775807,
		},
	},
}

func BenchmarkWriteStruct100(b *testing.B) {
	b.ReportAllocs()
	for n := 0; n < b.N; n++ {
		for _, testInput := range testData {
			buf := &bytes.Buffer{}
			buf.Reset()
			err := Write(buf, &testInput)
			if err != nil {
...
func BenchmarkReadWriteStruct100(b *testing.B) {
	b.ReportAllocs()
	for n := 0; n < b.N; n++ {
		testReadWriteStruct(b)
	}
}

func TestReadWriteStruct(t *testing.T) {
	testReadWriteStruct(t)
}
...
func testReadWriteStruct(t logger) {
	for _, testInput := range testData {
		buf := &bytes.Buffer{}
		err := Write(buf, &testInput)
		if err != nil {
		...
		dat := buf.Bytes()
		var outputBuffer bytes.Reader
		outputBuffer.Reset(dat)
		var testOutput sampleStructFormat
		if err := Read(&outputBuffer, &testOutput); err != nil {
			t.Fatalf("unable to Read: %+v", err)
		}
		if diff := DeepEqual(testInput, testOutput); diff != nil {
			t.Fatalf("Unable to write/read packet: %v", diff)
		}
	}
}
